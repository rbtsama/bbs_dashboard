...
- **链接功能**: 支持点击标题直接跳转到原帖
- **过滤功能**: 支持"不看我的帖子"选项，可以快速过滤掉已标记为"我的帖子"的内容
- **关注功能**: 支持将帖子添加到关注列表或标记为"我的帖子"
- **标记样式**: 使用不同颜色区分关注状态，绿色表示已关注，橙色表示我的帖子
...
1. **帖子排行榜增强功能**
   - 添加"不看我的帖子"过滤选项，支持快速筛选内容
   - 优化过滤逻辑，确保正确显示所需内容
   - 使用蓝色样式突出显示过滤选项，提升用户体验

2. **异动日志显示优化**
   - 使用 Timeline 组件替代表格，提供更直观的时间线展示
   - 简化异动类型为四种主要类型：重发、回帖、删回帖、新发帖
   - 为不同类型的异动添加颜色标识：蓝色(重发)、绿色(回帖)、红色(删回帖)、紫色(新发帖)
   - 优化日期时间格式，符合中文用户习惯

3. **关注列表功能增强**
   - 添加悬停预览功能，鼠标悬停在"查看异动"按钮上可快速查看最近10条异动记录
   - 优化"查看异动"按钮样式，使用主色调突出显示
   - 添加"查看更多"提示，当异动记录超过10条时引导用户查看完整记录
   - 简化异动日志显示，减少不必要的嵌套和边框
