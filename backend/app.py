...
def init_db():
    ...
    # 创建索引提高查询性能
    conn.execute('CREATE INDEX IF NOT EXISTS idx_thread_id ON posts(thread_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_post_time ON posts(post_time)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_scraping_time ON thread_history(scraping_time)')
